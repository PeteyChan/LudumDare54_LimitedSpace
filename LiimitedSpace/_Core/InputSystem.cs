...
public static partial class InputSystem
{
    ...
    public static bool isNumpad(this Inputs input) => (int)input >= (int)Inputs.key_pad_0 && (int)input <= (int)Inputs.key_pad_9;
    ...
    public static float CurrentValue(this Inputs input, Godot.Node node = null) => input_system.current[(int)input];
    public static float PreviousValue(this Inputs input, Godot.Node node = null) => input_system.previous[(int)input];
    static Input_Implemntation implemetation = new Input_Implemntation { Name = "Input System" };
    static Input_Implemntation.Impl input_system => Godot.Engine.IsInPhysicsFrame() ? implemetation.physics : implemetation.process;
    static InputSystem()
    {
        var tree = (SceneTree)Godot.Engine.GetMainLoop();
        tree.Root.AddChild(implemetation);
        implemetation.SetProcess(true);
        implemetation.SetPhysicsProcess(true);
    }
    ...
    partial class Input_Implemntation : Godot.Node
    {
        ...
        public override void _Notification(int what)
        {
            switch ((long)what)
            {
                case Godot.Node.NotificationProcess:
                    process.OnUpdate();
                    break;

                case Godot.Node.NotificationPhysicsProcess:
                    physics.OnUpdate();
                    break;
            }
        }
        ...
        public override void _Input(InputEvent @event)
        {
            ...
            {
                process.OnMouse(mouse_button);
                physics.OnMouse(mouse_button);
            }
        }
        ...
        public struct Impl
        {
            ...
            public void OnMouse(InputEventMouseButton @event)
            {
                if (@event.Pressed)
            ...
            float wheel_up = default, wheel_down = default;
